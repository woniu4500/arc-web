...
public final class HSSFXLSWriter {
	 ...
	 * @throws Exception
	 */
	private static <T> void createFooter(short startRow, HSSFSheet sheet,
		...
		for (CellDataType prop : propNames){
			if(prop.isFlag()){
				short rowNum = (short) (startRow+data.size());
				...
				long value = 0;
				for(T tData : data){
					if(PropertyUtils.getProperty(tData, prop.getPropName()) instanceof Long){
						value = (Long) PropertyUtils.getProperty(tData, prop.getPropName())+value; 
					}
					if(PropertyUtils.getProperty(tData, prop.getPropName()) instanceof String){
						value = new Long((String)PropertyUtils.getProperty(tData, prop.getPropName()))+value; 
					}

				}				
				HSSFCell cellTotal = row.createCell(i);
				cellTotal.getCellStyle().setDataFormat(HSSFDataFormat.getBuiltinFormat("0.00"));
				cellTotal.getCellStyle().setAlignment(HSSFCellStyle.ALIGN_RIGHT);
