...
//					double cellValue = NumberUtils.toLong(String.valueOf(val))/100.0 ;
					double cellValue = NumberUtils.toDouble(String.valueOf(val))/1.0 ;
			...
			case percent:
				...
				break;
			case month: 
				String monthVal = String.valueOf(val);
				cell.getCellStyle().setDataFormat(HSSFDataFormat.getBuiltinFormat("m/yy"));
				if ( monthVal.length() == 6 ) {
					cell.setCellValue(sdf8.parse(monthVal));
				}
				break;
			case defaultText: 
			default: 
