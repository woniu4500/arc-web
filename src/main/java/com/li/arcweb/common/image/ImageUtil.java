package com.li.arcweb.common.image;
...
public final class ImageUtil {
	...
	public static byte[] toByteArray(BufferedImage src, String type) throws IOException {
		...
		ImageIO.write(src, StringUtils.defaultString(type, DEFAULT_IMG_TYPE), os);
    ...
    public final static void zoomScale(BufferedImage src, OutputStream output, String type, double scale, boolean flag) {
        try {
            ...
            g.dispose();
            // 输出到文件流
            ImageIO.write(tag, StringUtils.defaultString(type, DEFAULT_IMG_TYPE), output);
        } catch (IOException e) {
    ...
    public final static void zoomFixed(BufferedImage src, OutputStream output, String type, int height, int width, boolean bb, Color fillColor) {
        try {
            ...
            ImageIO.write((BufferedImage) itemp, StringUtils.defaultString(type, DEFAULT_IMG_TYPE), output);
        } catch (IOException e) {
        	LOGGER.error("Error in zoom image", e);
        }
    }
            ...
            if (srcWidth > 0 && srcHeight > 0) {
                ...
                for (int i = 0; i < rows; i++) {
                    for (int j = 0; j < cols; j++) {
                        ...
                        g.dispose();
                        // 输出为文件
                        ImageIO.write(tag, StringUtils.defaultString(type, DEFAULT_IMG_TYPE), mos.buildOutputStream(i,j) );
                    }
                }
            }
        } catch (Exception e) {
    ...
    public final static void gray(BufferedImage src, OutputStream output, String type) {
        try {
            ...
            ImageIO.write(src, StringUtils.defaultString(type, DEFAULT_IMG_TYPE), output);
        } catch (IOException e) {
    ...
    public final static void textStamp(BufferedImage src, OutputStream output, String type, String text, Font font, Color color
        ...
        try {
            ...
            g.dispose();
            ImageIO.write((BufferedImage) image, StringUtils.defaultString(type, DEFAULT_IMG_TYPE), output);
        } catch (Exception e) {
